	...
	private static Pair<List<Integer>, Integer> caminoMaximo(BinaryTree<Integer> tree, Pair<List<Integer>, Integer> par, Pair<List<Integer>, Integer> max) {
		...
		switch (tree) {
			...
			case BTree<Integer> t -> {
				 ...
				 */
				List<Integer> izq = new ArrayList<Integer>(par.first());
				izq.add(t.label());
				max = caminoMaximo(t.left(), Pair.of(izq, par.second()*t.label()), max);
				List<Integer> der = new ArrayList<Integer>(par.first());
				der.add(t.label());
				max = caminoMaximo(t.right(), Pair.of(der, par.second()*t.label()), max);
			}
		}
		return max;
	}
