...
public class Ejemplo1 {
	...
	// Para hacerlo para arboles n-arios
	public static Boolean recursivo_nary(Tree<Character> tree, List<Character> ls, int i) {
		Integer n = ls.size();
		return switch (tree) {
		case TEmpty() -> false;
		case TLeaf(var lb) -> n - i == 1 && ls.get(i).equals(lb);
		case TNary(var lb, var ch) -> n - i > 0 && ls.get(i).equals(lb)
				&& ch.stream().anyMatch(c->recursivo_nary(c, ls, i+1));
		};
	}
}
